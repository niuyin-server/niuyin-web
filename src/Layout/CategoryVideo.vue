<template>
  <div class="flex flex-col">
    <!-- 分类子分类 -->
    <div v-if="videoCategoryChildrenList"
         class="flex gap-2 mb-4 overflow-x-auto whitespace-nowrap sticky top-0 z-10 py-2">
      <span
          ...
          <template #default>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4">
              ...
              <div
                  ...
                  :key="item.videoId"
                  class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer flex flex-col"
              >
                <!-- 视频封面区域 -->
                <div
                ...
                >
                  ...
                  <div v-if="!playingItems[item.videoId]" class="relative w-full h-full">
                    ...
                    >
                      <template #error>
                        <div
                            class="flex flex-col items-center justify-center h-full text-gray-400 bg-gray-100 dark:bg-gray-700">
...
import {ChatDotRound, Close, Picture, Star, User, View} from '@element-plus/icons-vue'
...
const currentPlayingId = ref(null) // 当前正在播放的视频唯一ID
...
// 生成唯一视频ID
const generateUniqueVideoId = (videoId, index) => {
...
// hover事件处理
const handleMouseEnter = async (item, index) => {
  const uniqueId = generateUniqueVideoId(item.videoId, index)

  // 停止当前正在播放的视频
  if (currentPlayingId.value && currentPlayingId.value !== uniqueId) {
    const currentPlayingVideoId = currentPlayingId.value.split('-')[0]
    playingItems[currentPlayingVideoId] = false
  }

  // 开始播放当前视频
  playingItems[item.videoId] = true
  currentPlayingId.value = uniqueId
...
const handleMouseLeave = (item, index) => {
  ...
  // 只有当前正在播放的视频才停止
  if (currentPlayingId.value === uniqueId) {
